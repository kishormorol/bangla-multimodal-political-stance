...
mT5               & 0.520 & 0.497 & 0.427--0.569 & 0.481 $\pm$ 0.078 & 0.591 & 0.470 \\
...
In this section, we present the performance of all evaluated models across textual, visual, multimodal, and proposed approaches. All headline results (Tables~\ref{tab:text_models} and~\ref{tab:multimodal_models}) come from the unified 5-fold CV protocol; Tables~\ref{tab:ml_models},~\ref{tab:image_models}, and~\ref{tab:proposed_models_compact} report preliminary single-split numbers from an earlier evaluation and are not directly comparable.
...
Table~\ref{tab:text_models} shows the performance of text models under 5-fold CV. BanglaBERT achieves the highest macro-F1 (0.533, CI 0.458--0.606), consistent with its Bangla-specific pretraining. A simple TF-IDF + Logistic Regression baseline (0.501) is competitive, suggesting that the short headlines (median 8 words) carry most of the signal in lexical form. mBERT and mT5 perform comparably (0.498, 0.497), while XLM-RoBERTa and Bangla-ELECTRA lag behind. The majority baseline (0.228) confirms that the task is non-trivial despite the class imbalance. Figure~\ref{fig:transformer_comparison} shows a visual comparison of all text models.  
...
Table~\ref{tab:ml_models} presents preliminary results of classical ML models from a single train-test split of augmented data. These numbers are \textbf{not directly comparable} to the 5-fold CV results; the TF-IDF + Logistic Regression baseline in Table~\ref{tab:text_models} (macro-F1 0.501) is the CV-validated classical baseline. Figure~\ref{fig:ml_comparison} shows a visual comparison of all classical ML models.  
...
Table~\ref{tab:image_models} presents preliminary results of image-only models from a single-split evaluation. These are \textbf{not directly comparable} to the CV results. ViT achieves the highest accuracy (0.497) among image-only models. Figure~\ref{fig:image_comparison} visually compares all image models.  


\begin{table*}[h]
\footnotesize
\centering
\caption{Performance of multimodal models under grouped stratified 5-fold CV ($n{=}86$, items with an image). All confidence intervals are wide due to the small evaluation set; ranking differences are not statistically meaningful.}
...
ALIGN         & dual encoder  & 0.419 & 0.407 & 0.294--0.510 & 0.376 $\pm$ 0.049 & 0.471 & 0.318 \\
...
\footnotesize
\centering
\caption{Text models re-scored on the 86-item image subset for apples-to-apples comparison with Table~\ref{tab:multimodal_models}.}
...
Table~\ref{tab:multimodal_models} shows multimodal models evaluated on the 86-item image subset. CLIP leads with macro-F1 0.516 (CI 0.396--0.627), followed by CountVec+ViT (0.431) and ALIGN (0.407). Dual-encoder models (CLIP, ALIGN) outperform fusion-encoder models (BLIP, ViLT, FLAVA), likely because the frozen dual-encoder architecture is more robust with a small training set. However, all confidence intervals overlap substantially, so ranking differences are not statistically reliable at this sample size. FLAVA's govt\_leaning F1 of 0.000 indicates complete failure on the smallest class. Figure~\ref{fig:multimodal_comparison} illustrates the comparison.  
...
Table~\ref{tab:proposed_models_compact} presents our six proposed multimodal fusion models from a preliminary single-split evaluation. These results are \textbf{not directly comparable} to the 5-fold CV results in Tables~\ref{tab:text_models} and~\ref{tab:multimodal_models}, as they use a different evaluation protocol. We include them for completeness while the unified CV evaluation of these architectures is in progress. Figure~\ref{fig:proposed_comparison} visually compares all six proposed models.  
...
\subsubsection{Zero-Shot LLM Baseline}
To calibrate the difficulty of the task, we evaluate a zero-shot LLM baseline: Qwen~3.8 27B, a multilingual model, is prompted with a task description and each headline in turn, with no training examples. The model achieves macro-F1 0.475 (CI 0.399--0.546, $n{=}195$), which is competitive with fine-tuned mBERT (0.498) and mT5 (0.497) despite having no task-specific training. The LLM performs well on govt\_critique (F1 0.66) but struggles with govt\_leaning (F1 0.28), mirroring the pattern of supervised models. This suggests that the task's difficulty lies primarily in distinguishing neutral from government-aligned framing, not in understanding Bangla per se.


\section{Conclusion and Future Work}
We presented a reproducible benchmark for Bangla multimodal political stance detection, evaluating 7 text models, 6 vision-language architectures, and a zero-shot LLM baseline under a unified evaluation protocol. BanglaBERT leads text models (macro-F1 0.533) while CLIP unfrozen is the strongest multimodal model (0.541), and a zero-shot Qwen~3.8 27B (0.475) nearly matches fine-tuned transformers without any training. Pairwise significance tests show the top four text models (BanglaBERT, mBERT, mT5, TF-IDF) form a statistically tied cluster, and most multimodal differences are not reliable at $n{=}86$. Ablations reveal that text augmentation has no measurable effect, encoder fine-tuning is critical for text models, and unfreezing multimodal encoders provides consistent gains.
